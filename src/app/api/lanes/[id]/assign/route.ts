...
export async function POST(
  ...
  try {
    ...
    const regularCount = currentAssignments.filter(a => a.lane.type === 'REGULAR').length
    const priorityCount = currentAssignments.filter(a => a.lane.type === 'PRIORITY').length

    // Business rule: User can have at most 2 assignments total.
    // Regular lanes up to 2 and Priority lanes up to 1.
    if (currentAssignments.length >= 2) {
      return NextResponse.json(
        { error: 'User can only be assigned to a maximum of 2 lanes' },
        { status: 400 }
      )
    }

    if (lane.type === 'REGULAR' && regularCount >= 2) {
      return NextResponse.json(
        { error: 'User can only be assigned to a maximum of 2 Regular lanes' },
        { status: 400 }
      )
    }

    if (lane.type === 'PRIORITY' && priorityCount >= 1) {
      return NextResponse.json(
